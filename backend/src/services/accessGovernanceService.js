...
async function completeCampaign(orgId, campaignId, completedBy) {
  ...
  const summaryPayload = {
    ...
    items: items.map((item) => ({
      ...
      // Explicit so a machine reading this evidence can separate independent
      // decisions from administrator self-certifications.
      self_reviewed: item.reviewer_id != null && String(item.reviewer_id) === String(item.subject_user_id),
