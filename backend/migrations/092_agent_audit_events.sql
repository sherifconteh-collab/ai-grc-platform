...
-- Composite index for daily summary queries. Avoid expression indexes on
-- TIMESTAMPTZ because date_trunc(...) is not immutable on PostgreSQL.
CREATE INDEX IF NOT EXISTS idx_agent_audit_events_date_outcome
    ON agent_audit_events(created_at DESC, outcome);
