...
- **Migration idempotency gaps**: added missing `IF NOT EXISTS` guards to 62 `CREATE TABLE`/`CREATE INDEX` statements across 5 migration files so a from-scratch `npm run migrate` run no longer fails partway through.
