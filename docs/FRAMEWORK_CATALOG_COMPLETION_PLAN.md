...
`nist_800_53` (300 — Wave 1 base-control item complete, see below).
...
1. **DONE — 800-53 base controls to 300.** Generated directly from NIST's
   official OSCAL catalog (`usnistgov/oscal-content`, Rev 5.2.0, public
   domain) rather than hand-typed; all 20 families now seed their complete
   non-withdrawn base-control set (enhancements still excluded, matching
   this repo's existing convention). Priority (`1`/`2`/`3`) is derived from
   official Low/Moderate/High baseline membership rather than guessed.
   `coverage_status` flips to `comprehensive` via migration 131 (applied
   both as a migration, for already-deployed databases, and re-applied by
   `seed-frameworks.js` itself on every seed run — see
   `lib/frameworks/coverageStatus.js` — since `nist_800_53` only exists as a
   seed-script row and migration 131 alone would be a no-op against a
   database that hasn't seeded yet). `seed-missing-controls.js`'s
   since-redundant 97-control NIST 800-53 backfill was folded into the
   module and removed to keep one source of truth. Items 2-4 below (FedRAMP
   baseline derivation, 800-171 Rev 3, CMMC 2.0 Rev-2 derivation, CSF 2.0
   top-up) remain open.
...
## Wave 2 — ISO family + SOC 2 (on hold — copyrighted source material)

**On hold:** this repo is not pursuing an ISO license, and per-Annex-control
completion is judged too close to reproducing the licensed standard's
structure/numbering to do without one. Not started as part of issue #217.
...
## Wave 3 — PCI DSS v4.0 + CIS Controls v8 (on hold — copyrighted source material)

**On hold:** same reasoning as Wave 2 — no PCI SSC or CIS license planned,
and full requirement/safeguard-level completion at this fidelity is judged
too close to reproducing the licensed standards without one. Not started as
part of issue #217.
